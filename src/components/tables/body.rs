...
impl Victim for OutputEnemy {
    fn champion_id(&self) -> ChampionId {
        self.champion_id
    }

    fn max_health(&self) -> i32 {
        self.current_stats.max_health as _
    }
...
fn attack_cells(damages: &Damages, cells: &mut Vec<Cell>, other: Option<&Damages>) {
    let Attacks {
        basic_attack,
        critical_strike,
        onhit_damage,
...
fn value_damage<T: ValueId>(
    ...
    let damages = match tag.entity() {
        ...
        EntityId::Item(_) => &damages.items,
        EntityId::Rune(_) => &damages.runes,
    };
...
pub fn render_cells(damages: &Damages, cells: Vec<Cell>) -> Html {
    debug_assert_eq!(cells.len(), cells.capacity());

    cells
        .into_iter()
        .map(|cell| cell.render(&damages.ctx))
        .collect::<Html>()
}
...
pub fn to_html(
    ...
    let mut cells = make_cells(champion_id, items_meta, runes_meta);

    if let Some(other) = other {
        debug_assert_eq!(other.items.len(), damages.items.len());
        debug_assert_eq!(other.runes.len(), damages.runes.len());
    }

    attack_cells(damages, &mut cells, other);
    abilities_damage(damages, &mut cells, champion_id, other);
    value_damage(
        damages,
        &mut cells,
        items_meta,
